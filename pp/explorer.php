    ...
    function fileSelected(valor){
        ...
        if(actualFile==name){
            ...
            return launchEditor("infofoto","http://dspcommunity.com/pp/image.php?i="+actualFile);
				...
				alert ("Only Facebook users have this privileges");
			}
            ...
            foreach($data["data"] as $screenData){
               foreach($screenData as $sElement){
                 if($sElement["type"]=="image" && strlen($sElement["value"])<150){
                    ...
                    <input type=\"hidden\" id=\"elementId$i\" value=\"".$sElement["elementId"]."\"><input type=\"hidden\" id=\"scenarioId$i\" value=\"".$data["scenarioId"]."\">
                    <input type=\"hidden\" id=\"dataId$i\" value=\"".$idData."\"></li>";
                    $i++;
                 }
               }
